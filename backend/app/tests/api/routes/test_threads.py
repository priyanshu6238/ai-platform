...
@patch("app.api.routes.threads.OpenAI")
def test_process_run_non_completed_status(mock_openai):
    """Test process_run when the run has a non-completed status."""
    # Setup mock client
    mock_client = MagicMock()
    mock_openai.return_value = mock_client

    # Simulate a run with non-completed status
    mock_run = MagicMock()
    mock_run.status = "requires_action"  # Using a different non-completed status
    mock_client.beta.threads.runs.create_and_poll.return_value = mock_run
    ...
    with patch("app.api.routes.threads.send_callback") as mock_send_callback:
        ...
        callback_url, payload = mock_send_callback.call_args[0]
        assert callback_url == request["callback_url"]
        assert "Run failed with status: requires_action" in payload["error"]
        assert payload["success"] is False
