...
def test_create_credential_invalid_provider(
    db: Session, superuser_token_headers: dict[str, str], create_organization_and_creds
):
    ...
    response = client.post(
        ...
        json={
            "organization_id": org.id,
            "credential": {"invalid_provider": {"api_key": "test-key"}},
        },
        headers=superuser_token_headers,
    )
    assert response.status_code == 400
...
def test_update_credential_invalid_provider(
    db: Session, superuser_token_headers: dict[str, str], create_organization_and_creds
):
    ...
    response = client.post(
        ...
        json={
            "organization_id": org.id,
            "credential": {"openai": {"api_key": "test-key"}},
        },
        headers=superuser_token_headers,
    )
    assert response.status_code == 200
    ...
    response = client.patch(
        f"{settings.API_V1_STR}/credentials/{org.id}",
        json={"provider": "invalid_provider", "credential": {"api_key": "new-key"}},
        headers=superuser_token_headers,
    )
    assert response.status_code == 400
    assert "Unsupported provider" in response.json()["error"]
